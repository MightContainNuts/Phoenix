```mermaid

graph TB;
    CEO[CEO: neil]-->COO[COO: callum]
    COO-->TeamAsst1[TeamAsst: q]
    COO-->TeamAsst2[TeamAsst: dave]
    CEO-->CFO[CFO: gillian]
    CEO-->CTO[CTO: dean]
    CTO-->UK_Teamlead_1[UK_Teamlead 1: gav]
    CTO-->UK_Teamlead_2[UK_Teamlead 2: tbd]
    CTO-->SG_Teamlead1[UK_Teamlead 1: michael]
    UK_Teamlead_1-->FieldEngineer_1.1[Field Engineer_1.1: tbd]
    UK_Teamlead_1-->FieldEngineer_1.2[Field Engineer_1.2: tbd]
    UK_Teamlead_1-->FieldEngineer_1.3[Field Engineer_1.3: tbd]
    UK_Teamlead_2-->FieldEngineer_1.1[Field Engineer_2.1: tbd]
    UK_Teamlead_2-->FieldEngineer_2.2[Field Engineer_2.2: tbd]
    UK_Teamlead_2-->FieldEngineer_2.3[Field Engineer_2.3: tbd]
    SG_Teamlead_3-->FieldEngineer_3.1[Field Engineer_3.1: tbd]
    SG_Teamlead_3-->FieldEngineer_3.2[Field Engineer_3.2: tbd]
    SG_Teamlead_3-->FieldEngineer_3.3[Field Engineer_3.3: tbd]

